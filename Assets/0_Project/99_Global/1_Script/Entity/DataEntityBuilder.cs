...
namespace TenMinute.Combat {
    public sealed partial class DataEntity {
        ...
        public static DataEntity 효과부여(EffectID id, int 부여량 = 1) {
            return new DataEntity(EntityType.효과부여, Effect.Create(id));
        }
        public static DataEntity 효과회수(EffectID id, int 회수량 = 1) {
            return new DataEntity(EntityType.효과회수, Effect.Create(id));
        }
